...
public class TaskPool implements Runnable{
	...
	private ArrayList<ITask> mQueue = null; 
	
	private Thread mThread;
	
	private TaskPool() {
		mQueue = new ArrayList<ITask>(); 
	 ...
	 * @param taskInfo
	 */
	public synchronized void addTask(ITask taskInfo) {
		mQueue.add(taskInfo);
	}
	 ...
	 * @param taskInfo
	 */
	public synchronized void removeTask(ITask taskInfo) {
	 ...
	 * @return
	 */
	public synchronized ITask requestNewTask() {
		int index = mQueue.size();
		if (index > 0) {
			/**按顺序依次执行，先进先出*/
			ITask task = mQueue.get(0);
			mQueue.remove(0);
			return task;
		} else {
			return null;
